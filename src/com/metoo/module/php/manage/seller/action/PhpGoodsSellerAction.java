package com.metoo.module.php.manage.seller.action;
...
public class PhpGoodsSellerAction {
	...
	@Autowired
	private AppGoodsViewTools mgoodsViewTools;
	...
	public void createGoodsFinish(HttpServletRequest request, HttpServletResponse response, String id,
		...
		if(user.getStore() != null){
			...
			if(goods_class_id != null){
				...
				GoodsClass gc = this.goodsClassService.getObjById(CommUtil.null2Long(goods_class_id));
				boolean m = false;
				...
				if (m) {
					...
					if (id == null || "".equals(id)) {
						...
						this.goodsSku(goods.getId(), specList);
						// 图片排序
						if (!temp_ids.isEmpty()) {
						...
						this.goodsSku(CommUtil.null2Long(id), specList);
						/*
				...
				result = new Result(400, "参数错误:商品分类为空:goods_class_id:" + goods_class_id);
			}
			
		}else{
			result = new Result(400, "参数错误:未注册店铺");
		}
		}else{
			result = new Result(400, "参数错误:用户为空:user_id:"+ user_id);
		}
		CommUtil.returnJson(Json.toJson(result, JsonFormat.compact()), response);
	}

	public void goodsSku(Long goodsId, List<Map> specList) {
		...
		if (obj.getGoodsSkuList().size() > 0) {
			...
			for (GoodsSku goodsSku : goodsSkuList) {
				boolean flag = this.goodsSkuService.delete(goodsSku.getId());
			}
			obj.getGoodsSkuList().clear();
		}
		for (Map spec : specList) {
			...
			BigDecimal goods_sku_price = CommUtil.null2BigDecimal(spec.get("price").equals("") ? 0 : spec.get("price"));
			BigDecimal goods_sku_current_price = CommUtil
			...
			if (!goods_sku_price.equals(goods_sku_current_price)) {
				...
				double discount_price = CommUtil.div(subtract, goods_sku_price) * 100;
				BigDecimal e = new BigDecimal(discount_price);
				goodsSku.setCgoods_discount_rate(e);
			...
			goodsSku.setGoods_inventory(CommUtil.null2Int(spec.get("count")));
			goodsSku.setCombination_id(CommUtil.null2String(spec.get("id")));
			goodsSku.setGoods_tiered_price(CommUtil.null2String(spec.get("goods_tiered_price")));
			...
			goodsSku.setGoods_serial(CommUtil.null2String(spec.get("goodsSerial")));
			goodsSku.setGoods_weight(CommUtil.null2Int(spec.get("weight")));
			goodsSku.setGoods_length(CommUtil.null2Int(spec.get("length")));
			goodsSku.setGoods_width(CommUtil.null2Int(spec.get("width")));
			goodsSku.setGoods_high(CommUtil.null2Int(spec.get("high")));
			goodsSku.setEid(CommUtil.null2String(spec.get("eid")));
			goodsSku.setGoods_disabled(CommUtil.null2String(spec.get("goodsDisabled")));
			goodsSku.setSpec_color(CommUtil.null2String(spec.get("spec_color")));
			if (CommUtil.null2Int(spec.get("goods_warn_inventory")) - CommUtil.null2Int(spec.get("count")) > 0) {
				goodsSku.setWarn_inventory_status(-1);
			} else {
				goodsSku.setWarn_inventory_status(0);
			}
		...
		if (user_id == null || user_id.equals("")) {
			...
			if (accessory != null) {
				...
				if (code == 0) {
					...
					for (GoodsSku goodsSku : accessory.getC_goods_list()) {
						goodsSku.getGoods_photos().remove(accessory);
	...
	public void batchEditOverseaInventory(HttpServletRequest request, HttpServletResponse response,
		...
		if (null != obj) {
			GoodsSku coogds = this.goodsSkuService.getObjById(CommUtil.null2Long(sku_id));
			if (null != coogds) {
				if (coogds.getGoods().getId().equals(obj.getId())) {
					coogds.setOversea_inventory(oversea_inventory);
					this.goodsSkuService.update(coogds);
					code = 5200;
					msg = "Successfully";
				} else {
					code = 5213;
			...
			if (type != null && !"".equals(type)) {
				...
				if (inventory_detail != null && obj.getInventory_type().equals("spec")
					...
					for (Object cid : map.keySet()) {
						GoodsSku sku = this.goodsSkuService.getObjById(CommUtil.null2Long(cid));
						if (null != sku) {
							...
							this.goodsSkuService.update(sku);
						}
					}
					// 计算键最小折扣率
