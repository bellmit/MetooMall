...
import com.metoo.module.app.manage.admin.tool.AppOrderFormTool;
...
public class AppOrderBuyerAction {
	@Autowired
	private ISysConfigService configService;
	@Autowired
	private IUserConfigService userConfigService;
	@Autowired
	private AppOrderFormTool orderFormMetooTools;
	@Autowired
	...
	@Autowired
	private AppGameTreeTools gameTreeTools;
	@Autowired
		...
		if (CommUtil.null2String(token).equals("")) {
			...
			if (user == null) {
				...
				if (obj != null && obj.getUser_id().equals(CommUtil.null2String(user.getId()))) {
					...
					result = new Result(4400, "The order does not exist");
				}
			}
		}
		response.setContentType("application/json; charset=utf-8");
	...
	@RequestMapping(value="order_pay.json")
	@ResponseBody
	public String order_pay(HttpServletRequest request, HttpServletResponse response, String id, String token) {
		int code = 4200;
		String msg = "Success";
		if (!CommUtil.null2String(token).equals("")) {
			User user = this.userService.getObjByProperty(null, "app_login_token", token);
			if (user != null) {
				OrderForm obj = this.orderFormService.getObjById(Long.valueOf(id));
				if (obj != null && obj.getOrder_main() == 1) {
					if ("online".equals(obj.getPayType()) && obj.getOrder_status() == 10) {
						...
						StoreLog storeLog = this.storeLogTools
								.getTodayStoreLog(CommUtil.null2Long(obj.getStore_id()));
						storeLog.setPlaceorder(storeLog.getPlaceorder() + 1);
						if (this.orderFormTools.queryOrder(obj.getStore_name())) {
						...
						code = 4406;
						msg = "Maliciously tampering with parameter information";
					}
				} else {
					code = 4400;
					msg = "parameter error";
			...
			if (user == null) {
				...
				if (obj != null && obj.getUser_id().equals(user.getId().toString())) {
					...
					for (OrderForm of : objs) {
						if (of.getOrder_status() >= 20 && of.getOrder_status() >= 65) {
							all_verify = false;
						}
					}
				}
				if (all_verify) {
					if (obj != null && obj.getUser_id().equals(user.getId().toString())) {
						...
						// 取消订单删除下单赠送的冻结积分金额
						// 返回抵用券
						List<GoodsVoucherInfo> goodsVoucherInfoList = obj.getGoodsVoucherInfoList();
						if(goodsVoucherInfoList.size() > 0){
							this.appGameAwardTools.editGoodsVoucherInfo(goodsVoucherInfoList);
						}
			...
			if (user == null) {
				...
				if (obj != null && obj.getUser_id().equals(CommUtil.null2String(user.getId()))) {
					...
					if (ret) {// 订单状态更新成功，更新相关信息 暂时不处理子订单
						...
						// 主订单生成商家结算日志
						if (obj.getOrder_form() == 0) {
							game.put("order_id", obj.getId());
							game.put("game_task_id", obj.getGame_task_id());
					...
					// 确认收货赠送水滴
					game.put("user_id", user.getId());
					String message = this.gameTreeTools.grantWater(game);
					...
					if(gameTasks.size() > 0){
						...
						gameAward = this.appGameAwardTools.createUpgradeAward(user, gameTask.getGameAward());
						// 订单等额奖励
						int water = obj.getPayment_amount().intValue();
						user.setWater_drops_unused(user.getWater_drops_unused() + water);
						this.userService.update(user);
						// 记录订单奖励通知
						GameTreeLog ltg = new GameTreeLog();
						ltg.setAddTime(new Date());
						ltg.setUser_id(user.getId());
						ltg.setUser_name(user.getUsername());
						ltg.setWater(water);
						ltg.setType(11);
						ltg.setStatus(11);
					...
					result = new Result(4200, "Success", gameAward);
				} else {
					result = new Result(1, "The order does not exist");
				}
			}
				...
				if (imglist.size() > 40) {// 每个用户最多可以上传40张临时图片，半小时清理一次
					...
					try {
						...
						if (map.get("fileName") == null) {
							...
							result = new Result(4200, "Success", json_map);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
				...
				if (accessory == null && accessory.getUser().getId().equals(user.getId())) {
					...
					if (code == 0) {
						...
						if (accessory.getC_goods_list().isEmpty()) {
							for (GoodsSku obj : accessory.getC_goods_list()) {
								obj.getGoods_photos().remove(accessory);
								this.goodsSkuService.update(obj);
							}
						}
	...
	/*
	 * @SecurityMapping(title = "买家评价保存", value = "/buyer_order_evaluate_save*",
	 * rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
	 * 
	 * @RequestMapping(value = "order_evaluate_save.json", method =
	 * RequestMethod.POST) public void order_evaluate_save(HttpServletRequest
	 * request, HttpServletResponse response, String id, String
	 * evaluate_session, String goods_id, String token) throws Exception {
	 * Result result = null; int code = -1; String msg = ""; if
	 * (CommUtil.null2String(token).equals("")) { code = -100; msg =
	 * "token Invalidation"; } else { Map params = new HashMap(); User user =
	 * this.userService.getObjByProperty(null, "app_login_token", token); if
	 * (user == null) { code = -100; msg = "token Invalidation"; } else {
	 * OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(id));
	 * String evaluate_session1 = (String)
	 * request.getSession(false).getAttribute("evaluate_session"); if
	 * (evaluate_session1 != null && evaluate_session1.equals(evaluate_session))
	 * { if
	 * (!CommUtil.null2String(request.getParameter("evaluate_info")).equals(""))
	 * { request.getSession(false).removeAttribute("evaluate_session"); if (obj
	 * != null && obj.getUser_id().equals(CommUtil.null2String(user.getId()))) {
	 * int i = obj.getOrder_status(); if (obj.getOrder_status() == 40) {
	 * List<Map> json = this.orderFormTools.queryGoodsInfo(obj.getGoods_info());
	 * boolean flag = false; int description =
	 * eva_rate(request.getParameter("description_evaluate")); int service =
	 * eva_rate(request.getParameter("service_evaluate")); int ship =
	 * eva_rate(request.getParameter("ship_evaluate")); int total =
	 * eva_rate(request.getParameter("evaluate_buyer_val")); if (description ==
	 * 0 || service == 0 || ship == 0 || total == -5) { flag = true; } if (flag)
	 * { result = new Result(3, "参数错误，禁止评价"); code = 4400; msg =
	 * "parameter error"; } else { OrderFormLog ofl = new OrderFormLog();
	 * ofl.setAddTime(new Date()); ofl.setLog_info("评价订单");
	 * ofl.setLog_user(user); ofl.setOf(obj);
	 * this.orderFormLogService.save(ofl); obj.setFinishTime(new Date());
	 * this.orderFormService.update(obj); for (Map map : json) {
	 * map.put("orderForm", obj.getId()); } // 暂时不处理子订单 List<Map> child_order =
	 * // this.orderFormTools.queryGoodsInfo(obj.getChild_order_detail()); //
	 * List<Map> child_goods = new // ArrayList<Map>(); // for (Map c :
	 * child_order) { // List<Map> maps = this.orderFormTools //
	 * .queryGoodsInfo(c.get("order_goods_info").toString()); // for (Map cmap :
	 * maps) { // cmap.put("orderForm", c.get("order_id")); // } //
	 * child_goods.addAll(maps); // } // if (child_goods.size() > 0) { //
	 * json.addAll(child_goods); // } for (Map map : json) { Goods goods =
	 * this.goodsService.getObjById(CommUtil.null2Long(goods_id)); if (goods !=
	 * null) { if (map.get("goods_id").toString().equals(goods_id)) { List<Map>
	 * map_list = this.orderFormTools.compare(obj.getGoods_info(), goods_id);
	 * obj.setGoods_info(Json.toJson(map_list, JsonFormat.compact())); int
	 * order_status = this.orderFormTools.finish(obj.getGoods_info(), goods_id);
	 * if (order_status == 1) { obj.setOrder_status(50);
	 * this.orderFormService.update(obj); } Evaluate eva = new Evaluate();
	 * goods.setEvaluate_count(goods.getEvaluate_count() + 1);
	 * eva.setAddTime(new Date()); eva.setEvaluate_goods(goods);
	 ...
	 * eva.setEvaluate_type("goods"); eva.setEvaluate_user(user);
	 * eva.setOf(this.orderFormService
	 * .getObjById(CommUtil.null2Long(map.get("orderForm"))));
	 * eva.setReply_status(0); this.evaluateService.save(eva); String im_str =
	 * request.getParameter("evaluate_photos"); if (im_str != null &&
	 * !im_str.equals("") && im_str.length() > 0) { for (String str :
	 * im_str.split(",")) { if (str != null && !str.equals("")) { Accessory
	 * image = this.accessoryService .getObjById(CommUtil.null2Long(str)); if
	 * (image.getInfo().equals("eva_temp")) { image.setInfo("eva_img");
	 * this.accessoryService.save(image); } } } } params.clear(); if
	 * (goods.getGoods_type() == 1) { Store store =
	 * this.storeService.getObjById(
	 * CommUtil.null2Long(goods.getGoods_store().getId()));
	 * params.put("store_id", store.getId().toString()); List<Evaluate> evas =
	 * this.evaluateService.query(
	 * "select obj from Evaluate obj where obj.of.store_id=:store_id", params,
	 * -1, -1); double store_evaluate = 0; double description_evaluate = 0;
	 * double description_evaluate_total = 0; double service_evaluate = 0;
	 * double service_evaluate_total = 0; double ship_evaluate = 0; double
	 * ship_evaluate_total = 0; DecimalFormat df = new DecimalFormat("0.0"); for
	 * (Evaluate eva1 : evas) { description_evaluate_total =
	 * description_evaluate_total +
	 * CommUtil.null2Double(eva1.getDescription_evaluate());
	 * service_evaluate_total = service_evaluate_total +
	 * CommUtil.null2Double(eva1.getService_evaluate()); ship_evaluate_total =
	 * ship_evaluate_total + CommUtil.null2Double(eva1.getShip_evaluate()); }
	 * description_evaluate = CommUtil.null2Double(
	 * df.format(description_evaluate_total / evas.size())); service_evaluate =
	 * CommUtil.null2Double( df.format(service_evaluate_total / evas.size()));
	 * ship_evaluate = CommUtil .null2Double(df.format(ship_evaluate_total /
	 * evas.size())); store_evaluate = (description_evaluate + service_evaluate
	 * + ship_evaluate) / 3;// 综合评分=三项具体评分之和/3 store.setStore_credit(
	 * store.getStore_credit() + eva.getEvaluate_buyer_val());
	 * this.storeService.update(store); params.clear(); params.put("store_id",
	 * store.getId()); List<StorePoint> sps = this.storePointService.query(
	 * "select obj from StorePoint obj where obj.store.id=:store_id", params,
	 * -1, -1); StorePoint point = null; if (sps.size() > 0) { point =
	 * sps.get(0); } else { point = new StorePoint(); } point.setAddTime(new
	 * Date()); point.setStore(store);
	 * point.setDescription_evaluate(BigDecimal.valueOf( description_evaluate >
	 * 5 ? 5 : description_evaluate)); point.setService_evaluate(BigDecimal
	 * .valueOf(service_evaluate > 5 ? 5 : service_evaluate));
	 * point.setShip_evaluate( BigDecimal.valueOf(ship_evaluate > 5 ? 5 :
	 * ship_evaluate)); point.setStore_evaluate(BigDecimal
	 * .valueOf(store_evaluate > 5 ? 5 : store_evaluate)); if (sps.size() > 0) {
	 * this.storePointService.update(point); } else {
	 * this.storePointService.save(point); } } else { User sp_user =
	 * this.userService.getObjById(obj.getEva_user_id()); params.put("user_id",
	 * user.getId().toString()); List<Evaluate> evas =
	 * this.evaluateService.query(
	 * "select obj from Evaluate obj where obj.of.user_id=:user_id", params, -1,
	 * -1); double store_evaluate = 0; double description_evaluate = 0; double
	 * description_evaluate_total = 0; double service_evaluate = 0; double
	 * service_evaluate_total = 0; double ship_evaluate = 0; double
	 * ship_evaluate_total = 0; DecimalFormat df = new DecimalFormat("0.0"); for
	 * (Evaluate eva1 : evas) { description_evaluate_total =
	 * description_evaluate_total +
	 * CommUtil.null2Double(eva1.getDescription_evaluate());
	 ...
	 * ship_evaluate = CommUtil .null2Double(df.format(ship_evaluate_total /
	 * evas.size())); store_evaluate = (description_evaluate + service_evaluate
	 * + ship_evaluate) / 3; params.clear(); params.put("user_id",
	 * obj.getEva_user_id()); List<StorePoint> sps =
	 * this.storePointService.query(
	 * "select obj from StorePoint obj where obj.user.id=:user_id", params, -1,
	 * -1); StorePoint point = null; if (sps.size() > 0) { point = sps.get(0); }
	 * else { point = new StorePoint(); } point.setAddTime(new Date());
	 * point.setUser(sp_user); point.setDescription_evaluate(BigDecimal.valueOf(
	 * description_evaluate > 5 ? 5 : description_evaluate));
	 * point.setService_evaluate(BigDecimal .valueOf(service_evaluate > 5 ? 5 :
	 * service_evaluate)); point.setShip_evaluate(
	 * BigDecimal.valueOf(ship_evaluate > 5 ? 5 : ship_evaluate));
	 * point.setStore_evaluate(BigDecimal .valueOf(store_evaluate > 5 ? 5 :
	 * store_evaluate)); if (sps.size() > 0) {
	 * this.storePointService.update(point); } else {
	 * this.storePointService.save(point); } } this.goodsService.update(goods);
	 * User usr = this.userService
	 * .getObjById(CommUtil.null2Long(obj.getUser_id()));
	 * usr.setIntegral(usr.getIntegral() +
	 * this.configService.getSysConfig().getIndentComment()); // 增加用户消费金额
	 * usr.setUser_goods_fee(BigDecimal.valueOf(
	 * CommUtil.add(usr.getUser_goods_fee(), obj.getTotalPrice())));
	 * 
	 * this.userService.update(usr); // 记录积分明细 if
	 * (this.configService.getSysConfig().isIntegral()) { IntegralLog log = new
	 * IntegralLog(); log.setAddTime(new Date()); log.setIntegral_user(usr);
	 * log.setType("Add"); log.setIntegral_from("order"); int integral =
	 * this.configService.getSysConfig() .getMemberCharEvaluate(); if (null ==
	 * request.getParameter("evaluate_photos") ||
	 * request.getParameter("evaluate_photos").equals("")) {
	 * log.setIntegral(integral); } else { String photo =
	 * request.getParameter("evaluate_photos"); String[] str = photo.split(",");
	 * double pintegral = CommUtil.mul(str.length,
	 * this.configService.getSysConfig().getMemberPhotoChar()); integral += new
	 * Double(pintegral).intValue(); log.setIntegral(integral); }
	 * log.setContent("订单文字评价增加" + integral + "分");
	 * this.integralLogService.save(log); } code = 4200; msg = "Success"; }
	 * } else { code = 4205; msg = "The commodity does not exist"; }
	 * 
	 * if (this.configService.getSysConfig(). isEmailEnable()) { if
	 * (obj.getOrder_form() == 0) { Store store = this.storeService.getObjById(
	 * CommUtil. null2Long(obj.getStore_id())); Map map = new HashMap();
	 * map.put("seller_id", store.getUser().getId().toString());
	 * map.put("order_id", obj.getId().toString()); String jsonmap =
	 * Json.toJson(map);//发送邮件 this.msgTools.sendEmailCharge( CommUtil.
	 * getURL(request), "email_toseller_evaluate_ok_notify",
	 * store.getUser().getEmail(), jsonmap, null, obj.getStore_id()); } }
	 * 
	 * } } } else { code = 2; msg = "This order is not evaluable"; } } } else {
	 * code = 4400; msg = "parameter error"; } } else { code = 1; msg =
	 * "No repeat evaluation"; } } } result = new Result(code, msg); try {
	 * response.getWriter().print(Json.toJson(result, JsonFormat.compact())); }
	...
	public void order_return_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		if (token.equals("")) {
			...
			if (user == null) {
				...
				result = new Result(4200, "Success", returnList);
			}
		}
		response.setContentType("application/json");
		response.setCharacterEncoding("UTF-8");
				...
				if (orderForm != null) {
					if (orderForm.getUser_id().equals(user.getId().toString())) {
						...
						if (obj != null) {
							...
							result = new Result(4200, "Success", return_apply_map);
						} else {
	 ...
	 * @param order_status
	 * @param column
	 *            private final static String xls = "xls"; private final static
	 *            String xlsx = "xlsx";
	 * @descript 订单状态修改, 不同版本使用poi不同实现类 当前版本支持*.xls
	 * 
	 */
	...
	public void brushBatch(HttpServletRequest request, HttpServletResponse response, MultipartFile file,
		...
		if (null != file) {
			try {
				...
				XSSFSheet xsheet1 = xwb.getSheet("Sheet1");
	...
	/*
	 * @RequestMapping("verify_email.json") public void
	 * verifyEmail(HttpServletRequest request, HttpServletResponse respones,
	 * String token) { int code = -1; String msg = "";
	 * if(!CommUtil.null2String(token).equals("")){ User user =
	 * this.userService.getObjByProperty(null, "app_login_token", token);
	 * if(user != null){ if(user.getEmail().equals("")){
	 * 
	 * }else{ code = 4200; } }else{
	 * 
	 * } } }
