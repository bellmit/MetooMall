...
import com.metoo.foundation.domain.ArticleDto;
...
public class AppArticleViewAction {
	@Autowired
	...
	@ResponseBody
	public Object unread(String token){
		Map params = new HashMap();
		params.put("mark", "Discovery");
		List<Article> articles = this.articleService
				.query("SELECT new Article(obj.id, obj.addTime, obj.title, obj.type, obj.url, obj.content, obj.article_id, obj.article_acc, obj.click, obj.love, obj.accessory) "
	...
	@ResponseBody
	public Object article(HttpServletRequest request, HttpServletResponse response, String token, Integer currentPage,
			Integer pageSize) {
		...
		try {
			...
			if(language.equals("sa")){
				articles = this.articleService
						.query("SELECT new Article(obj.id, obj.addTime, obj.title, obj.title_sa, obj.type, obj.url, obj.content_sa, obj.article_id, obj.article_acc, obj.click, obj.love, obj.accessory) "
								+ "FROM Article obj " + "WHERE obj.mark=:mark "
								+ "ORDER BY obj.sequence DESC, obj.addTime DESC", params, (currentPage - 1) * pageSize, pageSize);
			}else{
				articles = this.articleService
			...
			// 热点文章
			List<Article> articlesHots = this.articleService.query(
					"SELECT new Article(obj.id, obj.title, obj.click, obj.love, obj.article_acc, obj.accessory) " 
							+ "FROM Article obj "
							+ "WHERE obj.mark=:mark " 
							+ "ORDER BY obj.love DESC",
					params, 0, 4);
			// map.put("articlesHots", articlesHots);
			...
			if(user != null){
				...
				params.put("user_id", user.getId());
				params.put("type", 1);
				List<ArticleInfo> articleInfos = this.articleInfoService
						.query("SELECT obj FROM ArticleInfo obj WHERE obj.user.id=:user_id "
								+ "AND obj.type=:type", params, -1, -1);
				if(articleInfos.size() > 0){
					HashSet set = new HashSet(articleInfos);
					articleInfos.clear();
					// 将去重后的元素重新添加到list中
					articleInfos.addAll(set);
					if(articles.size() > 0){
						for(Article article : articles){
							article.setFlag(true);
							for(ArticleInfo articleInfo : articleInfos){
								if(article.getId().equals(articleInfo.getArticle().getId())){
	...
	/*
	 * public void footer_api(HttpServletRequest request, HttpServletResponse
	 * response, String currentPage) { Result result; Map<String, Object> map =
	 * new HashMap<String, Object>(); ModelAndView mv = new JModelAndView("",
	 ...
	 * , params, -1, -1);
	 * 
	 * ArticleQueryObject aqo = new ArticleQueryObject(currentPage, mv,
	 * "addTime", "desc"); aqo.addQuery("obj.mark", new SysMap("mark",
	 * "Discovery"), "="); aqo.setPageSize(10);
	 * aqo.setCurrentPage(CommUtil.null2Int(currentPage)); IPageList pList =
	 * this.articleService.list(aqo); List<Article> articles =
	 * pList.getResult(); List<Map<String, Object>> articleList = new
	 * ArrayList<Map<String, Object>>(); for (Article article : articles) {
	 * Map<String, Object> articleMap = new HashMap<String, Object>();
	 * articleMap.put("url", article.getUrl()); articleMap.put("id",
	 * article.getId()); articleMap.put("title", article.getTitle());
	 * articleMap.put("articleid", article.getArticle_id()); articleMap
	 * .put("article_acc", article.getArticle_acc() == null ? "" :
	 * this.configService.getSysConfig().getImageWebServer() + "/" +
	 * article.getArticle_acc().getPath() + "/" +
	 * article.getArticle_acc().getName()); articleMap.put("articlegcinfo",
	 * wap_article(article.getArticle_id())); articleList.add(articleMap); }
	 * map.put("articlelist", articleList); map.put("article_Pages",
	 * pList.getPages()); result = new Result(0, "Succes", map);
	...
	public void article(HttpServletRequest request, HttpServletResponse response, String param, String token, String language) {
		Result result = null;
		User user = null;
		...
		if (id != -1l) {
			...
			if (obj != null) {
				...
				map.put("love", obj.getLove());
				map.put("click", obj.getClick());
				// 查询用户是否点赞
				// 查询用户是否已对该文章点赞
				if(user != null){
					Map params = new HashMap();
					params.put("user_id", user.getId());
					params.put("article_id", obj.getId());
					params.put("type", 0);
					
					List<ArticleInfo> articleInfos = this.articleInfoService.query("SELECT obj "
							+ "FROM ArticleInfo obj "
							+ "WHERE obj.user.id=:user_id "
							+ "AND obj.article.id=:article_id "
							+ "AND obj.type=:type", params, -1, -1);
					int flag = 0;
					if(articleInfos.size() > 0){
						flag = 1;
				...
				List<List> article_goods_list = new ArrayList<List>();
				if (goodsFloor != null && goodsFloor.getChilds().size() > 0) {
					for (GoodsFloor gf : goodsFloor.getChilds()) {
						if (gf.isGf_display()) {
							...
							article_goods_list.add(goods_list);
						}
					}
				}
				map.put("article_goods", article_goods_list);
	...
	public void article_goods(HttpServletRequest request, HttpServletResponse response, String gc_id,
		...
		result = new Result(4200, "Success", wap_map);
		response.setContentType("application/json");
		response.setCharacterEncoding("UTF-8");
		try {
			response.getWriter().print(Json.toJson(result, JsonFormat.compact()));
	 ...
	 * @return
	 */
	@RequestMapping("/click.json")
	@ResponseBody
	public Object click(String token, String id) {
		User user = null;
		if (CommUtil.isEmpty(token)) {
			user = this.userService.getObjByProperty(null, "app_login_token", token);
		}
	...
	@ResponseBody
	public Object like(String token, String id) {
		User user = null;
		if (CommUtil.isEmpty(token)) {
			user = this.userService.getObjByProperty(null, "app_login_token", token);
		}
