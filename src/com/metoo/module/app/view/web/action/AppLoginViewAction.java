...
/**
 * <p>
 * 	Title: AppLoginViewAction.java
 * </p>
 * 
 * <p>
 * 	Description: 可使用session，存入用户session,使用File判断用户是否登录或使用SpringSecurity；我就不修改了，，留给你了
 * </p>
 * @author 46075
 *
 */
...
*/					//if (usr_name != null && !usr_name.equals("") && usr_password != null && !usr_password.equals("")) {
			...
			if (flag) {
				if (this.configService.getSysConfig().isSmsEnbale()) {
					...
					if (CommUtil.null2String(map.get("areaMobile")).equals("88888888")) {
						...
						String content = "Your Metoomall verification code is " + code + ".The code is valid in 5 mins.";
	...
	public void app_forget(HttpServletRequest request, HttpServletResponse response, String mobile,
			String mobile_verify_code, String password) {
		int code = -1;
		String msg = "";
		...
		if (flag) {
			if (!config.isEmailEnable() && !config.isSmsEnbale()) {
				...
				if (mvc != null && mvc.getCode().equalsIgnoreCase(mobile_verify_code)) {
					...
					params.put("userName",  map.get("areaMobile").toString());
					params.put("mobile", map.get("userMobile").toString());
					List<User> users = this.userService
							.query("select obj from User obj where obj.userName =:userName or obj.mobile =:mobile", params,
							-1, -1);
					if (users.size() > 0) {
						...
						if (ret) {
							String content = "Dear customer " 
											+ user.getUsername()
											+ ", you have success changed your password." 
						...
						code = 3;
						msg = "Please enter the registered phone number";
					}
