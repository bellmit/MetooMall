...
public class AppCartViewTools {
		...
		if (carts.size() > 0) {
			...
			for (GoodsCart obj : carts) {
				...
				if (goods.getGoods_store().getId().equals(id)) {
					...
					if (!CommUtil.null2String(obj.getSpec_info()).equals("") || !CommUtil.null2String(obj.getColor()).endsWith("")) {
						goods_main_photo = this.configService.getSysConfig().getImageWebServer() + "/"
								+ this.generic_default_info_color(obj.getGoods(), obj.getCart_gsp(), obj.getColor())
										.get("accessory").toString();
					} else {
						goods_main_photo = obj.getGoods().getGoods_main_photo() == null ? ""
								: this.configService.getSysConfig().getImageWebServer() + "/"
			...
			if ("spec".equals(goods.getInventory_type())) {
				...
				for (GoodsSku obj : goodsSkuList) {
					...
					if (Arrays.equals(gsp_ids, s_id) && obj.getGoods_disabled().equals("0")
						...
						if (obj.getC_goods_photos().size() > 0) {
							for (Accessory photo : obj.getC_goods_photos()) {
								if (photo != null) {
									accessory = photo.getPath() + "/" + photo.getName() + "_small." + photo.getExt();
									break;
								}
							}
						}
	...
	public void buyNow(HttpServletRequest request, HttpServletResponse response, Map map, Goods obj, String area_abbr,
		...
		// 商品信息
		Map goodsMap = this.appGoodsViewTools.orderGoods(obj, count, gsp, color, point, language);
	...
	public void buyNowV2(HttpServletRequest request, HttpServletResponse response, Map map, Goods obj, String area_abbr,
		...
		boolean point = true; // 兑换活动商品 obj.getGoods_status() == 4
		// 商品信息
		Map goodsMap = this.appGoodsViewTools.orderGoodsV2(obj, count, gsp, color, language);
		Map priceMap = this.buyNowEnoughReducePrice(obj, gsp, color, count, type);
		goods_price = CommUtil.null2Double(priceMap.get("goods_price"));
	 ...
	 * @decript 购物车商品
	 */
	public void cart_order(List map_list, String area_id, String user_id, Map cartMap, String language, Boolean voucher, double total_ship_price) {
		...
		double ship_subsidy_price = 0;// 平台满包邮与店铺总价差价
		double order_ship_price = 0;// 平台满包邮价格(订单运费总金额：各个店铺运费之和)--（方式二：改为所有店铺商品重量之和）
		double order_free_express_price = 0;// 平台满包可使用订单金额
		List<Map> orderlist = new ArrayList<Map>();
		for (int i = 0; i < map_list.size(); i++) {
			Map storeMap = new HashMap();
		...
		if (this.configService.getSysConfig().getEnoughfree_status() == 1) {
			...
			if (CommUtil.subtract(order_goods_amount, order_free_express_price) >= 0) {
				...
				// 店铺商品重量
				order_total_price = CommUtil.add(order_goods_amount, total_ship_price);
		...
		if(goodsVoucherPrice > 0 && voucher){
			order_total_price = CommUtil.subtract(order_total_price, goodsVoucherPrice) >= 0 ? CommUtil.subtract(order_total_price, goodsVoucherPrice) : 0;
		}
...
//		cartMap.put("order_ship_price", order_ship_price);
		cartMap.put("order_ship_price", total_ship_price);
		cartMap.put("order_total_price",order_total_price);
		cartMap.put("orderlist", orderlist);
		cartMap.put("goodsVoucherPrice", goodsVoucherPrice);
		cartMap.put("ship_flag", ship_flag);
	}
	...
	public void cart_order_adjust(List map_list, String area_abbr, String area_id, String user_id, Map cartMap,
			String generalCouponId, String language, String integral, Boolean voucher, double total_ship_price) {
...
//				order_discounts_amount += order_ship_price;
				order_discounts_amount += total_ship_price;
//				order_ship_price = 0;
				ship_flag = true;
			} else if (order_ship_price > 0) {
				// 店铺商品重量之和
//				order_total_price = CommUtil.add(order_total_price, order_ship_price);
				order_total_price = CommUtil.add(order_total_price, total_ship_price);
				ship_subsidy_price = CommUtil.subtract(order_free_express_price, order_goods_amount);
			}
		}
		// 抵用券
...
//		cartMap.put("order_ship_price", order_ship_price);
	...
	public List cartGoodsV2(List<GoodsCart> carts, String language) {
		...
		for (GoodsCart gc : carts) {
			...
			String goods_img = this.configService.getSysConfig().getImageWebServer() + "/"
					+ gc.getGoods().getGoods_main_photo().getPath() + "/"
					+ gc.getGoods().getGoods_main_photo().getName();
			int goods_inventory = 0;
			if (gc.getGoods().getInventory_type().equals("spec")) {
				...
				goods_img = this.configService.getSysConfig().getImageWebServer() + "/"
						+ this.generic_default_info_color(gc.getGoods(), gc.getCart_gsp(), gc.getColor())
								.get("accessory").toString();
			} else {
				goods_inventory = gc.getGoods().getGoods_inventory();
			}
	...
	//
	public double calCartPrice_free(List<GoodsCart> carts, String gcs) {
		double all_price = 0.0;
		Map<String, Double> ermap = new HashMap<String, Double>();
		if (CommUtil.null2String(gcs).equals("")) {
			for (GoodsCart gc : carts) {
				if(gc.getGoods().getGoods_transfee() == 0){
					if (null != gc.getCart_type() & "0".equals(gc.getCart_type()) || "1".equals(gc.getCart_type())) {// 普通商品处理
						// if (gc.getCart_type().equals("app") ||
						// gc.getCart_type().equals("web")) {// 普通商品处理
						all_price = CommUtil.add(all_price, CommUtil.mul(gc.getCount(), gc.getPrice()));
	...
	public List<GoodsCart> cartListCalc(HttpServletRequest request, User user, String cart_session_id) {
		...
		if (user != null) {
			...
			if (!"".equals(cart_session_id)) {
				...
				params.put("cart_status1", 2);
				params.put("deleteStatus", 0);
				userCartList = this.goodsCartService
						.query("select obj from GoodsCart obj where obj.user.id=:user_id and obj.deleteStatus=:deleteStatus and obj.cart_status=:cart_status or obj.cart_status=:cart_status1 "
								+ "and obj.deleteStatus=:deleteStatus", params, -1, -1);
			} else {
				params.clear();
				params.put("user_id", user.getId());
				params.put("cart_status", 0);
			...
			if (!"".equals(cart_session_id)) {
				...
				params.put("cart_status1", 2);
				params.put("deleteStatus", 0);
				cookieCartList = this.goodsCartService
						.query("select obj from GoodsCart obj where obj.cart_session_id=:cart_session_id and obj.deleteStatus=:deleteStatus and obj.cart_status=:cart_status or obj.cart_status=:cart_status1 "
								+ "and obj.deleteStatus=:deleteStatus", params, -1, -1);
			}
		}
	...
	public void updateGoodsInventory(OrderForm order) {
		...
		for (Goods obj : goodsList) {
			...
			if (inventory_type.equals("all")) {
				...
				GoodsSku cobj = this.orderFormTools.queryChildGoods(obj, gsp, color);
				if (cobj != null) {
					if (flag) {
					...
					this.goodsSkuService.update(cobj);
				}
				obj.setGoods_inventory(obj.getGoods_inventory() - goods_count);
			}
			// 更新商品秒杀状态
	 ...
	 * @return
	 */
	public BigDecimal exchange_rate(String payment_platform, BigDecimal total_price){
		Map params = new HashMap();
		params.put("mark", payment_platform);
		...
		if(payments.size() > 0){
			...
			if(exchange_rate > 0){
				double price = CommUtil.mul(total_price, exchange_rate);
//				double price_integer = Math.ceil(price);
