...
@Component
public class AppGoodsSimilarTools {
	...
	public Set<Map> recommendGoods(ModelAndView mv, String currentPage, int remaining, Map similar_map,
			String language) {
		GoodsQueryObject gqo = new GoodsQueryObject("new Goods(id, goods_name, goods_price, goods_current_price, weightiness_rate, well_evaluate, goods_discount_rate, ksa_goods_name, goods_status, goods_main_photo)", currentPage, mv, "weightiness", ",rand()");
		if (remaining != 0) {
	 ...
	 * @throws SQLException
	 */
	public Set<Map> key_value(String currentPage, List<String> keys, List<String> values, Map<String, Integer> countMap,
			String appendMulitId) throws SQLException {
		String sql = "";
		int num = 0;
		...
		if (!sql.equals("")) {
			...
			properties.add("goods_vice_id");
			properties.add("goods_name");
			properties.add("ksa_goods_name");
			properties.add("goods_price");
			properties.add("goods_current_price");
			properties.add("goods_status");
			properties.add("goods_discount_rate");
			goodsSet = this.databaseTools.select_goods(query, properties);
			/*Long goods_id;
			String name = "";
			String ksa_name = "";
			String img;
	...
	public String union_sql(Long id, int number) {
		...
		String union_sql = "select goods_vice_id,goods_name, ksa_goods_name,goods_price,goods_current_price,be_similar,goods_status,goods_main_photo_id,goods_discount_rate from ("
				...
				+ "goods_main_id=" + id + " order by metoo_goods.weightiness desc" + ") table1";
		return union_sql;
	}
