		...
		if (!CommUtil.null2String(json).equals("")) {
			try {
				...
				for (Map map : list) {
					...
					Goods obj = this.goodsService.getObjById(CommUtil.null2Long(map.get("goods_id")));
					...
					fpv.setFpv_goods_name(CommUtil.null2String(map
							.get("goods_name")));
					fpv.setFpv_ksa_goods_name(CommUtil.null2String(map
							.get("ksa_goods_name")));
					...
					fpv.setFpv_goods_current_price(BigDecimal.valueOf(CommUtil
							.null2Double(map.get("goods_current_price"))));
					fpv.setFpv_goods_discount_rate(BigDecimal.valueOf(CommUtil
							.null2Double(map.get("goods_discount_rate"))));
					fpv.setFpv_goods_class_id(CommUtil.null2Long(map
							.get("goods_class_id")));
					fpv.setFpv_goods_class_name(CommUtil.null2String(map
							.get("goods_class_name")));
					fpv.setFpv_goods_status(obj.getGoods_status());
					fpv.setFpv_store_status(obj.getGoods_store().getStore_status());
					fpvs.add(fpv);
				}
			} catch (Exception e) {
				e.printStackTrace();
