...
public class GoodsFloorManageAction {
	...
	public ModelAndView goods_floor_list_goods_load(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String gc_id,
			String goods_name, String page, String module_id, String goods_serial) {
		ModelAndView mv = new JModelAndView(
		...
		if(CommUtil.null2String(goods_serial).equals("")){
			...
			List<GoodsSku> clist = this.goodsSkuService.query("select obj from GoodsSku obj where obj.goods_serial=:goods_serial", params, -1, -1);
			if(!clist.isEmpty()){
				for(GoodsSku cg : clist){
