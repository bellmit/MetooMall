...
public class OrderFormTools {
		...
		if (!CommUtil.null2String(of.getChild_order_detail()).equals("")) {
			...
			for (Map map : maps) {
				...
				for (Map map1 : map_list) {
					Goods goods = this.goodsService.getObjById(CommUtil.null2Long(map1.get("goods_id")));
					goods_list.add(goods);
				}
			}
		}
	 ...
	 * @return
	 */
	public GoodsSku queryChildGoods(Goods goods, String gsp, String color) {
		double price = 0;
		Map map = new HashMap();
		int count = 0;
		String sku = "";
		...
		for (Goods obj : goodsList) {
			if (null != obj) {
				...
				if (inventory_type.equals("all")) {
					...
					GoodsSku cobj = this.queryChildGoods(obj, gsp, color);
					if (cobj != null) {
						if (flag) {
