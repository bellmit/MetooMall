...
public class CommUtil {
	...
	public static String showPageFormHtml(int currentPage, int pages) {
		...
		if (pages > 0) {
			...
			if (beginPage <= pages) {
				...
				for (int i = beginPage, j = 0; i <= pages && j < 6; i++, j++)
					if (i == currentPage)
						...
						s += "<a href='javascript:void(0);' title='第" + i + "页' onclick='return gotoPage(" + i +
			...
			if (currentPage <= pages) {
				...
				s += "<a class='btn_last' href='javascript:void(0);' title='末页' onclick='return gotoPage(" + pages
						+ ")'>Last</a> ";
			}
		}
		// s+=" 转到<input type=text size=2>页";
	...
	public static final String randomIntApp(int length) {
		...
		char[] randBuffer = new char[length];
		for (int i = 0; i < randBuffer.length; i++) {
			randBuffer[i] = numbersAndLetters[randGen.nextInt(20)];// nextInt:这里是一个方法的重载，参数的内容是指定范围
		}
		return new String(randBuffer);
	}
	...
	public static Map httpsaveFileToServer(HttpServletRequest request, String filePath, String saveFilePathName,
		...
		if (commonsMmultipartFile != null && !commonsMmultipartFile.isEmpty()) {
			...
			if (flag) {
				...
				httpClient = HttpClients.createDefault();// 创建
															// CloseableHttpClient
				HttpPost httpPost = new HttpPost(path);
				try {
					String httpfilePath = "/" + saveFilePathName;
					MultipartEntityBuilder entityBuilder = MultipartEntityBuilder.create();// 向一个Http实体添加成分
																							// 是向一个HttpEntity添加成分来表示表单的一般方法.
