...
public class GoodsViewTools {
	...
	public GoodsLog getTodayGoodsLog(long id) {
		try {
			...
			cal.setTime(now);
			cal.set(Calendar.HOUR_OF_DAY, 0);//将给定的日历字段设置为给定的值
	...
	public UserRecommend getTodayUserRcommend(Long id, Long user_id){
		Map params = new HashMap();
		Date data = new Date();
		Calendar cal = Calendar.getInstance();
		cal.setTime(data);
		...
		cal.set(Calendar.MINUTE, 0);
		cal.set(Calendar.SECOND, 0);
		params.put("begin_time", cal.getTime());
		data = cal.getTime();
		cal.add(Calendar.DAY_OF_YEAR, 1);
		params.put("end_time", cal.getTime());
		params.put("goods_id", id);
		params.put("user_id", user_id);
		List<UserRecommend> userRecommends = this.userRecommendService
				.query("select obj from UserRecommend obj where obj.goods_id=:goods_id and obj.user_id=:user_id "
						+ "and obj.addTime>=:begin_time and obj.addTime<:end_time", params, -1, -1);
		if(userRecommends.size() == 0){
			...
			userRecommend.setAddTime(new Date());
			userRecommend.setGoods_id(obj.getId());
			userRecommend.setGoods_name(obj.getGoods_name());
			userRecommend.setCategory(obj.getGc().getId());
			userRecommend.setUser_id(user_id);
			userRecommend.setUserName(user.getUserName());
			userRecommend.setStore_id(obj.getGoods_store().getId());
			userRecommend.setGoods_brand_id(obj.getGoods_brand() == null ? null : obj.getGoods_brand().getId());
