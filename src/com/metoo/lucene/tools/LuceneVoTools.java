...
public class LuceneVoTools {
	...
	public LuceneVo updateGoodsIndex(Goods goods) {
		...
		vo.setVo_goods_weightiness(CommUtil.null2Double(goods.getWeightiness()));
		...
		if (goods.getGoods_property() != null && !goods.getGoods_property().equals("")) {
			List<Map> map_list = new ArrayList<Map>();
			map_list = Json.fromJson(ArrayList.class, goods.getGoods_property());
			for (Map map : map_list) {
				Iterator<Map.Entry<String, String>> it = map.entrySet().iterator();
				while (it.hasNext()) {
					Map.Entry<String, String> entry = it.next();
					strs.add(CommUtil.null2String(entry.getValue()));
				}
			}
		}
		vo.setVo_goods_property(strs.size() > 0 ? Json.toJson(strs) : "");
		// 查询商品正在进行的优惠活动
